...
"""
支持任意进制，在给定符号表后，对于常见的运算符，都进行运算符重载，以支持原有的
函数。包括，算数,比较，逻辑等等
额外说明，一些运算符，只有分母为1才可以使用如% //
四个独特方法

self.浮点数(截断位数) 带()标记循环节，在截断位数的范围内出现循环节的情况下
返回字符串形如"A.3EF(F2)" ,"3.A4..."这是16进制下的
返回的字符串中的无限小数要么带有循环节标记，要么带有...表示截断
有限小数是普通的"B.32"

这个是静态方法
self.有理数逼近(字符串,进制,符号表)
如果不输入后两项会使用默认值
使用最佳有理逼近的方法
完全支持self.浮点数的返回
返回一个多进制有理数对象
...
class 多进制有理数:
    ...
    def _验证进制和符号表(self):
        ...
        if not isinstance(self._符号表, str):
            raise TypeError("符号表必须是")
        if "." in self._符号表:
            raise ValueError("符号表中不允许有'.'")
        if "-" in self._符号表:
            raise ValueError("符号表中不允许有'-'")
    ...
    def _验证输入字符串(self, 字符串: str):
        ...
        if not all(字符 in 字符集合 for 字符 in 清理后字符):
            raise ValueError(
                f"输入字符串{字符串}包含非法字符！当前进制{self._进制}的合法字符为：{''.join(字符集合)}")
    ...
    @staticmethod
    def 有理数逼近(字符串:str, 进制 = 默认进制, 符号表 = 默认符号表,截断系数 = 100):
        def 连分数展开(p, q):
            """
            计算最简正分数 p/q 的连分数展开
        ...
        def 连分数截断(列表, 截断因子):
            ...
            参数:
            列表: 连分数系数列表
            截断因子: 相对倍数阈值，当前系数是前一个系数的factor倍时截断
